 ...
 */
export interface BasicMonitorOptions {
  filter?: DataFilterList;
  additional_kwargs?: MonitorCheckConfSchema;
}
...
export const getCheckReferenceApiV1ChecksCheckIdRunReferencePost = (
  checkId: number,
  basicMonitorOptions: BasicMonitorOptions
) =>
  customInstance<unknown>({
    url: `/api/v1/checks/${checkId}/run/reference`,
    method: 'post',
    headers: { 'Content-Type': 'application/json' },
    data: basicMonitorOptions
  });
...
>;
export type GetCheckReferenceApiV1ChecksCheckIdRunReferencePostMutationBody = BasicMonitorOptions;
...
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof getCheckReferenceApiV1ChecksCheckIdRunReferencePost>>,
    TError,
    { checkId: number; data: BasicMonitorOptions },
    TContext
  >;
}) => {
  const { mutation: mutationOptions } = options ?? {};
 ...
 * @summary Retrieve Model Version
 */
export const retrieveModelVersionApiV1ModelVersionsModelVersionIdGet = (modelVersionId: number, signal?: AbortSignal) =>
  customInstance<ModelVersionSchema>({ url: `/api/v1/model-versions/${modelVersionId}`, method: 'get', signal });

export const getRetrieveModelVersionApiV1ModelVersionsModelVersionIdGetQueryKey = (modelVersionId: number) => [
  `/api/v1/model-versions/${modelVersionId}`
];

export type RetrieveModelVersionApiV1ModelVersionsModelVersionIdGetQueryResult = NonNullable<
  Awaited<ReturnType<typeof retrieveModelVersionApiV1ModelVersionsModelVersionIdGet>>
>;
export type RetrieveModelVersionApiV1ModelVersionsModelVersionIdGetQueryError = ErrorType<HTTPValidationError>;

export const useRetrieveModelVersionApiV1ModelVersionsModelVersionIdGet = <
  TData = Awaited<ReturnType<typeof retrieveModelVersionApiV1ModelVersionsModelVersionIdGet>>,
  TError = ErrorType<HTTPValidationError>
>(
  modelVersionId: number,
  options?: {
  ...
  const queryKey =
    queryOptions?.queryKey ?? getRetrieveModelVersionApiV1ModelVersionsModelVersionIdGetQueryKey(modelVersionId);

  const queryFn: QueryFunction<Awaited<ReturnType<typeof retrieveModelVersionApiV1ModelVersionsModelVersionIdGet>>> = ({
    signal
  }) => retrieveModelVersionApiV1ModelVersionsModelVersionIdGet(modelVersionId, signal);
