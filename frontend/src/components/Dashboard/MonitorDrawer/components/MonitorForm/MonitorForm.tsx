...
function MonitorFormComponent({
  ...
  const { refetch: refetchMonitors } = useGetOrCreateDashboardApiV1DashboardsGet({
    query: {
  ...
  const saveMonitor = useCallback(async () => {
    ...
    monitor ? setMonitorToRefreshId(monitor.id) : refetchMonitors();
    formik.resetForm();
    onClose();
  }, [
    column,
...
export const MonitorForm = memo(MonitorFormComponent);
