import { Alert, Avatar, Box, Button, Dialog, DialogTitle, InputAdornment, List, ListItem, ListItemAvatar, ListItemText, Snackbar, TextField } from "@mui/material";
...
export const UserInviteDialog = ({ open, onClose } : PropsWithChildren<UserInviteDialogProps>) => {
    ...
    const handleInviteToOrgClick = () => {
      if (btnEnabled) {
        ...
        createInviteApiV1OrganizationInvitePut(inviteSchema).then(res => {
          setSuccess(true);
          handleClose();
        }).catch(reason => {
          console.log(reason);
      ...
      <Dialog 
        ...
        <Box sx={{ display: 'flex', justifyContent: 'center', m: 2, p: 2 }}>
            <TextField 
                ...
                onChange={text => validateEmail(text.target.value) }></TextField>
        ...
        <List sx={{ pt: 0 }}>
          {organizationMembers.map(member => (
            <ListItem button onClick={() => handleListItemClick(member.full_name ? member.full_name : member.email)} key={member.email}>
              <ListItemAvatar>
