...
const ModelDetails = ({ model }: ModelDetailsProps) => {
  ...
  const { data: versions, isLoading} = useRetriveConnectedModelVersionsApiV1ConnectedModelsModelIdVersionsGet(model.id);
  ...
  return (
    <Box sx={{ width: '100%' }}>
      ...
      <TabContext value={value}>
          ...
          <Grid container spacing={0.5} columns={4}>
            {versions?.map(version => (
              <>
                ...
                <Grid item xs={1}>
                  <Typography sx={{color: '#94A4AD'}}>
                    Pending rows: {version.n_of_pending_rows}
                  </Typography>
                </Grid>
                <Grid item xs={1}>
        ...
        <TabPanel value="2">
          Congrats! You have found the first easter egg on Deepchecks. Keep looking :) 
        </TabPanel>
      </TabContext>
      }
    </Box>
