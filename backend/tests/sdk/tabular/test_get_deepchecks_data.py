...
async def test_classification_model_production_data_retrieval(
    ...
    dataset, y_pred, y_proba = multiclass_model_version_client.get_deepchecks_production_dataset(
        start_time=start_time,
        end_time=end_time.add(hours=1)
    )

    df: pd.DataFrame = dataset.data
    df[SAMPLE_PRED_COL] = pd.Series(y_pred)
    df[SAMPLE_PRED_PROBA_COL] = [None] * (len(y_pred))

    for i, proba in enumerate(y_proba):
        df[SAMPLE_PRED_PROBA_COL][i] = proba.tolist()
...
def test_classification_model_production_data_retrieval_with_filter(
    ...
    dataset = multiclass_model_version_client.get_deepchecks_production_dataset(
    ...
    assert len(df) == 3
    assert df['a'].apply(lambda x: x >= 12).all(), df['a']
...
async def test_classification_model_reference_data_retrieval(
    ...
    dataset, y_pred, y_proba = multiclass_model_version_client.get_deepchecks_reference_dataset()
...
def test_classification_model_reference_data_retrieval_with_filter(
    ...
    dataset = multiclass_model_version_client.get_deepchecks_reference_dataset(
    ...
    assert len(df) == 1
    is_a_bigger = df['a'].apply(lambda x: x > 2)
