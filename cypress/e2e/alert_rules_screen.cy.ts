...
describe("Alert Rules Screen", () => {
    ...
    function fillBasicDetails(details: BasicDetails, submit: boolean = true) {
        return (form: JQuery<HTMLElement>) => {
            ...
            if (submit === true)
                cy.get("@basicDetailsSubmit").click();
        };
    };
    ...
    function fillMonitorData(monitorData: MonitorData, submit: boolean = true) {
        return (form: JQuery<HTMLElement>) => {
            ...
            if (monitorData.aggregationWindow !== undefined) {
                cy.wrap(form)
                    .contains("label", "Aggregation window")
                    .next()
                    .click();
                cy.get("div[role=presentation]#menu- li")
            ...
            if (submit === true)
                cy.get("@monitorDataSubmit").click();
        };
    };
    ...
    function fillAlertLogic(alertRuleLogic: AlertRuleLogic, submit: boolean = true) {
        return (form: JQuery<HTMLElement>) => {
            ...
            cy.wrap(form)
                .find("canvas[role=img]")
                .should("exist");
            
            if (alertRuleLogic.operator !== undefined) {
                cy.wrap(form)
                    .contains("label", "Select Operator")
                    .next()
                    .click();
                cy.get("div[role=presentation]#menu- li")
            ...
            if (submit === true)
                cy.get("@newAlertRuleSubmit").click();
        };
    }
    ...
    function findAlertRule(
        rule: {
            details: BasicDetails, 
            monitorData?: MonitorData,
        }
    ) {
        return cy.contains("p", rule.details.name)
            .should("exist")
            .parent()
            .parent()
            .parent()
        ...
        return findAlertRule(oldRule)
            ...
            .then(() => {
                ...
                if (newRule.details !== undefined) {
                    cy.get("@currentStepForm")
                        .then(fillBasicDetails(newRule.details));
                } else {
                    cy.get("@currentStepForm")
                        .contains("button", "Next")
                        .should("exist")
                ...
                if (newRule.monitorData !== undefined) {
                    cy.get("@currentStepForm")
                        .then(fillMonitorData(newRule.monitorData));
                } else {
                    cy.get("@currentStepForm")
                        .contains("button", "Next")
                        .should("exist")
                ...
                if (newRule.alertLogic !== undefined) {
                    cy.get("@currentStepForm")
                        .then(fillAlertLogic(newRule.alertLogic));
                } else {
                    cy.get("@currentStepForm")
                        .contains("button", "Save")
                        .should("exist")
    ...
    it("Check that models appear in the filtering dropdown", () => {
        cy.createModelAndVersion(availableModels[0].name, availableModels[0].taskType, availableModels[0].versionName)
            .then(modelInfo => {
                cy.addDataToVersion(modelInfo);
                return cy.addCheck(
                    modelInfo, 
                    {name: availableModels[0].checkName}
                )
            })
            .then(checkInfo => cy.addMonitor(checkInfo));
    
        cy.createModelAndVersion(availableModels[1].name, availableModels[1].taskType, availableModels[1].versionName)
            .then(modelInfo => {
                cy.addDataToVersion(modelInfo);
                return cy.addCheck(
                    modelInfo,
                    {name: availableModels[1].checkName}
                )
            })
            .then(checkInfo => cy.addMonitor(checkInfo));
    ...
    it("Test alert rule creation", () => {
        ...
        availableAlertRules.forEach(it => {
            chain
            .then(() => createNewAlertRule(it))
            .then(() => findAlertRule(it));
        });
    });
    ...
    it("Test alert rule deletion", () => {
        ...
        .then(() => {
            cy.contains("p", availableAlertRules[0].details.name)
                .should("not.exist");
        })
    });
